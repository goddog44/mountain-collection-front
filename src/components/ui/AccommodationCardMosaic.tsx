...
export default function AccommodationCardMosaic({ accommodation }: Props) {
  ...
  if (
    distanceToSlopes !== undefined &&
    distanceToSlopes < 400 &&
    !displayAmenities.some((a) => a.id === "proche-pistes")
  ) {
    displayAmenities.unshift({
      id: "proche-pistes",
      label: "Proche des pistes",
    });
  ...
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-[12px] bg-[var(--ts-white)] shadow-[0_0_12px_rgba(0,0,0,0.08)] transition-all duration-200 hover:shadow-[0_0_16px_rgba(0,0,0,0.12)]"
    >
      {/* IMAGE */}
        ...
        <div className="absolute left-2 top-2 flex flex-col gap-1.5">
          ...
          {(coupDeCoeur || !promoBadge) && (
            <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-white shadow-sm">
              <Home
                className="h-4 w-4 text-[var(--ts-text-card)]"
                strokeWidth={2}
              />
            </span>
          )}
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="mb-1 flex items-start justify-between gap-2">
          <h3 className="text-[1rem] font-bold leading-tight text-[var(--ts-text-card)]">
            {name}
          </h3>
        ...
        {price.freeCancellation && (
          <p className="mt-2 text-xs text-[var(--ts-success)]">
            Annulation gratuite
          </p>
        )}
      </div>
