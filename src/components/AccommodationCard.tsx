...
export default function AccommodationCard({ 
  ...
  const displayAmenities = amenities.slice(0, 4).map((a) => ({
    id: a.id,
  ...
  if (distanceToSlopes !== undefined && distanceToSlopes < 400 && !displayAmenities.some((a) => a.id === "proche-pistes")) {
    ...
    if (displayAmenities.length > 4) displayAmenities.pop();
  }
    ...
    return (
      ...
      >
        <div className="relative md:w-[280px] lg:w-[320px] flex-shrink-0">
          <ImageCarousel images={images} alt={name} className="h-full overflow-hidden rounded-t-[12px] md:rounded-l-[12px] md:rounded-tr-none" />
        ...
        <div className="flex flex-1 flex-col p-4 justify-between">
          <div>
            <div className="mb-1 flex items-start justify-between gap-2">
              <h3 className="text-[1.125rem] font-bold leading-tight text-[var(--ts-text-card)]">{name}</h3>
              <button
                type="button"
            ...
            {description && (
              <p className="mb-2 line-clamp-2 text-[0.875rem] text-[var(--ts-mid-grey)]">
                {description}
              </p>
            )}
            <p className="mb-3 text-[0.875rem] text-[var(--ts-text-card)]">
            ...
            <div className="flex flex-wrap gap-1.5">
              {displayAmenities.map((a) => (
                <span
                  key={a.id}
                  className="inline-flex items-center rounded-md bg-[var(--ts-light-blue)] px-2 py-1 text-[0.8rem] font-medium text-[var(--ts-mid-blue)]"
                >
                  {a.label}
                </span>
              ))}
          ...
          <div className="mt-4 flex items-end justify-end gap-2">
            <div className="text-right">
              ...
              <span className="text-xl font-bold text-[var(--ts-mid-blue)]">
